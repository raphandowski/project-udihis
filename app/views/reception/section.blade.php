...
<p>Doctor</p>
<select id="opd" class="form-control" name="doctor_id" required>  
<?php 
...
<select id="insurence" class="form-control" name="company" required>  
<option  value=""></option>
...
@if($section == "IPD")
<p>Doctor</p>
<select id="ipd" class="form-control" name="doctor_id" required>  
...
@if($section == "ANC")
<p>Doctor</p>
<select id="ipd" class="form-control" name="sectioninfo" required>  
<?php $docts = User::where('level',4)->get(); ?>
@foreach($docts as $dr)
<option name="sectioninfo" value="{{$dr->id}}">{{$dr->first_name}} {{$dr->last_name}}</option>
@endforeach
